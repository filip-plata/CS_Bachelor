...
//			transition.setToX(thumbArea.getLayoutX());
//			transition.setByX(-(thumbAreaWidth - thumbWidth));
		} else {
//			transition.setToX(thumbArea.getLayoutX() + thumbAreaWidth - thumbWidth);
//			transition.setByX(thumbAreaWidth - thumbWidth);
	...
	protected double computeMinHeight(double width, double topInset, double rightInset, double bottomInset,
		...
		final Font font = label.getFont();
		final String labelText = label.getText();
		final double textHeight = Utils.computeTextHeight(font, labelText, 0, label.getLineSpacing(),
				label.getBoundsType());
