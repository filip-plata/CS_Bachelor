...
void Server::send_reply(uint32_t last_event, ConnInfo conn) {
    ...
    try {
        ...
        ConnInfo client = *clients.begin();
        remove_client(client);
        send_reply(last_event, conn);
    }
}
...
void Server::GameState::helper_check_player_position(std::pair<const ConnInfo, Player> &it) {
